...
software="nginx apache2 apache2-utils apache2.2-common
	...
	libapache2-mod-fcgid libapache2-mod-php7.1 php7.1 php7.1-common
	php7.1-cgi php7.1-mysql php7.1-curl php7.1-fpm php7.1-pgsql awstats
	...
	php7.1-gd php7.1-intl php7.1-imagick php7.1-ldap php7.1-imap 
	php7.1-pspell php7.1-mbstring php7.1-sqlite3 php7.1-tidy php7.1-xdebug 
	php7.1-zip  php7.1-mcrypt php7.1-xmlrpc php7.1-xsl php-geoip php7.1-dev
	nano iftop atop nload git-core sshpass rar unrar unzip libreoffice memcached
	duplicity ncftp lftp python python-dev
	python-paramiko python-pycryptopp python-boto make gcc
	dialog libssl-dev libffi-dev librsync-dev ca-certificates"
	
#после установки (apt-get -y install $software) мои комманды 
...
sudo a2enmod remoteip php7.1 rewrite ssl headers mpm_prefork mpm_itk expires
sudo phpenmod -v 7.1 pdo_mysql mysqlnd pdo_sqlite mcrypt
...
sed -i 's/;phar.require_hash = On/phar.require_hash = Off/g' /etc/php/7.1/cli/php.ini
sed -i 's/;phar.require_hash = On/phar.require_hash = Off/g' /etc/php/7.1/apache2/php.ini
...
sed -i 's/max_input_time = 60/max_input_time = 180/g' /etc/php/7.1/apache2/php.ini
sed -i 's/post_max_size = 8M/post_max_size = 200M/g' /etc/php/7.1/apache2/php.ini
sed -i 's/upload_max_filesize = 2M/upload_max_filesize = 200M/g' /etc/php/7.1/apache2/php.ini
sed -i 's/max_file_uploads = 20/max_file_uploads = 150/g' /etc/php/7.1/apache2/php.ini

#активация модулей vesta
sed -i "s/LANGUAGE='en'/LANGUAGE='ru'/g" /usr/local/vesta/conf/vesta.conf
echo "FILEMANAGER_KEY='mykey'" >> /usr/local/vesta/conf/vesta.conf
echo "SFTPJAIL_KEY='mykey'" >> /usr/local/vesta/conf/vesta.conf
sed -i "s/\$BIN\/v-check-vesta-license/#\$BIN\/v-check-vesta-license/g" /usr/local/vesta/bin/v-backup-users
sed -i "s/v_host=/#v_host=/g" /usr/local/vesta/bin/v-activate-vesta-license
sed -i "s/answer=/#answer=/g" /usr/local/vesta/bin/v-activate-vesta-license
sed -i "s/check_result/#check_result/g" /usr/local/vesta/bin/v-activate-vesta-license
sed -i '/E_CONNECT/a\
answer=0' /usr/local/vesta/bin/v-activate-vesta-license

#дополнительне профили с Access-Control-Allow-Origin *;
cp /usr/local/vesta/data/templates/web/nginx/hosting.tpl /usr/local/vesta/data/templates/web/nginx/hosting_open.tpl
cp /usr/local/vesta/data/templates/web/nginx/hosting.stpl /usr/local/vesta/data/templates/web/nginx/hosting_open.stpl
cp /usr/local/vesta/data/templates/web/nginx/caching.tpl /usr/local/vesta/data/templates/web/nginx/caching_open.tpl
cp /usr/local/vesta/data/templates/web/nginx/caching.stpl /usr/local/vesta/data/templates/web/nginx/caching_open.stpl
cp /usr/local/vesta/data/templates/web/nginx/default.tpl /usr/local/vesta/data/templates/web/nginx/default_open.tpl
cp /usr/local/vesta/data/templates/web/nginx/default.stpl /usr/local/vesta/data/templates/web/nginx/default_open.stpl
sed -i '/Access-Control-Allow-Origin/d' /usr/local/vesta/data/templates/web/nginx/*_open.*tpl #удаляем
sed -i '/server_name/a\
    add_header Access-Control-Allow-Origin *;' /usr/local/vesta/data/templates/web/nginx/*_open.*tpl

#исправляем REMOTE_ADDR на реальный + добавляем переменные
sed -i '/.error.log/a\
    RemoteIPHeader X-Forwarded-For\
    SetEnv TMP /home/test/tmp\
    SetEnv TEMP /home/test/tmp\
    SetEnv TMPDIR /home/test/tmp' /usr/local/vesta/install/*/*/templates/web/apache2/*.*tpl
sed -i '/.error.log/a\
    RemoteIPHeader X-Forwarded-For\
	SetEnv TMP /home/test/tmp\
	SetEnv TEMP /home/test/tmp\
	SetEnv TMPDIR /home/test/tmp' /usr/local/vesta/data/templates/web/apache2/*.*tpl

#sys_temp_dir
sed -i '/php_admin_value session.save_path %home%\/%user%\/tmp/a\
        php_admin_value sys_temp_dir %home%\/%user%\/tmp' /usr/local/vesta/install/*/*/templates/web/apache2/*.*tpl
sed -i '/php_admin_value session.save_path %home%\/%user%\/tmp/a\
        php_admin_value sys_temp_dir %home%\/%user%\/tmp' /usr/local/vesta/data/templates/web/apache2/*.*tpl

#открываем доступ в папку /tmp
sed -i "s/:%home%\/%user%\/tmp/:%home%\/%user%\/tmp:\/tmp/g" /usr/local/vesta/install/*/*/templates/web/apache2/*.*tpl
sed -i "s/:%home%\/%user%\/tmp/:%home%\/%user%\/tmp:\/tmp/g" /usr/local/vesta/data/templates/web/apache2/*.*tpl
